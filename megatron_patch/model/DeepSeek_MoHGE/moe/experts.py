...
from ..mlp import MLP, MLPSubmodules, apply_swiglu_sharded_factory, ExpertMLP
...
class MyGroupedMLP(MegatronModule):
    """An efficient implementation of the Experts layer using GroupedGEMM.

    Executes multiple experts in parallel to maximize computational efficiency.
    """

    def __init__(self, group_indices, num_local_experts: int, config: TransformerConfig):
        super().__init__(config=config)
        self.config: TransformerConfig = config
        self.num_local_experts = num_local_experts
        gg.assert_grouped_gemm_is_available()
        ...
        fc1_output_size = self.config.ffn_hidden_size[group_indices] * self.num_local_experts
        if config.gated_linear_unit:
            # Project to 4h. If using swiglu double the output width,
            # see https://arxiv.org/pdf/2002.05202.pdf
            fc1_output_size *= 2
        ...
        fc2_input_size = self.config.ffn_hidden_size[group_indices] * self.num_local_experts
        fc2_input_size_per_partition = divide(fc2_input_size, tp_size)
...
class MyTEGroupedMLP(MegatronModule):
    """An efficient implementation of the Experts layer using TE's GroupedLinear.

    Executes multiple experts in parallel to maximize computational efficiency.
    """

    def __init__(self, group_indices, num_local_experts, config: TransformerConfig, submodules: MLPSubmodules):
        super().__init__(config=config)
        self.moe_extended_tp = config.moe_extended_tp
        self.num_local_experts = num_local_experts
        self.input_size = self.config.hidden_size
        ...
        # Double the output width with gated linear unit, see https://arxiv.org/pdf/2002.05202.pdf
        ffn_hidden_size = self.config.ffn_hidden_size[group_indices]
        if self.config.gated_linear_unit:
            ffn_hidden_size *= 2
        ...
        self.linear_fc2 = build_module(
            submodules.linear_fc2,
            self.num_local_experts,
            self.config.ffn_hidden_size[group_indices],
            self.config.hidden_size,
            config=self.config,
            init_method=self.config.output_layer_init_method,
            bias=self.config.add_bias_linear,
...
class MySequentialMLP(MegatronModule):
    """An implementation of the Experts layer using a sequence of MLP layers.

    This class executes each expert sequentially.
    """

    def __init__(self, local_expert_indices: list, num_local_experts: int, config: TransformerConfig, submodules: MLPSubmodules):
        super().__init__(config=config)
        self.add_bias = config.add_bias_linear
        self.moe_extended_tp = config.moe_extended_tp
        self.num_local_experts = num_local_experts
        ...
        for i in range(self.num_local_experts):
            group_indices = int((self.config.num_moe_expert_groups + ((local_expert_indices[i] % self.config.num_moe_expert_groups + 1) // 2) * pow(-1, local_expert_indices[i] % 2)) % self.config.num_moe_expert_groups)
            # group_indices = int((self.config.num_moe_expert_groups + ((local_expert_indices[i] + 1) // 2) * pow(-1, local_expert_indices[i] % 2)) % self.config.num_moe_expert_groups)
            expert = ExpertMLP(group_indices, self.config, submodules, is_expert=True)
            self.local_experts.append(expert)
