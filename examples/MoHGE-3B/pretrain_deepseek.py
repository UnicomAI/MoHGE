...
from megatron_patch.model.DeepSeek_MoHGE.layer_specs import (
    get_gpt_layer_with_transformer_engine_spec,
)
from megatron_patch.model.DeepSeek_MoHGE.model import GPTModel
from megatron_patch.model.DeepSeek_MoHGE.transformer_config import MyMoETransformerConfig
...
def model_provider(
    ...
    config = core_transformer_config_from_args(args, MyMoETransformerConfig)
    use_te = args.transformer_impl == "transformer_engine"

    config.attention_backend = 1
    if config.num_layers == 15:
        config.moe_ffn_hidden_size = [384, 512, 640, 768, 896, 1024, 1152, 1280]
        config.min_moe_ffn_hidden_size = 384
        config.moe_ffn_hidden_size_interval = 1024
        config.num_experts = 64
        config.router_topk = 6
        config.moe_group_router_topk = 4
        config.num_moe_expert_groups = 8
        config.moe_group_router_load_balancing_type = "aux_loss"
        config.moe_group_router_score_function = "sigmoid"
        config.moe_router_score_function = "softmax"
        config.moe_group_router_enable_expert_bias = False
        config.moe_router_enable_expert_bias = False
        config.micro_batch_size = 1
        config.moe_group_aux_loss_coeff = 1e-5

    print_rank_0(f"MoHGE config: {config}")

    if use_te:
        print_rank_0("building MoHGE model with Transformer Engine...")
        ...
        raise ValueError("MoHGE currently supports only Transformer Engine")
    ...
    print_rank_0('Number of parameter: % .3fB' % (total / 1e9))
